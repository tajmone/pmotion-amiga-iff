; source: https://www.purebasic.fr/french/viewtopic.php?p=175687#p175687

;----------------------------------------------------------
; Name:        Module TinyIFF
; Description: A tiny module for loading IFF images.
; Author:      flype, flype44(at)gmail(dot)com
; Revision:    1.2 (2015-09-11)
;----------------------------------------------------------
...
; http://amiga.lychesis.net/knowledge/EHB.html
;----------------------------------------------------------
...
Module TinyIFF
  ...
  Procedure UnPackBits(*bmhd.IFF_BMHD, *packedBits.BYTES, packedSize)
    
    Protected i, j, k, n, unpackedSize, *unpackedBits.BYTES
  ...
  Procedure UnInterleaveBits(*bmhd.IFF_BMHD, *interleaveBits.UBYTES)
    
    Protected C0, C1, C2, C3, C4, C5, C6
    Protected bytesPerRow, bit, x, y, z, *unInterleaveBits.UBYTES
    
    *unInterleaveBits = AllocateMemory(*bmhd\w * *bmhd\h * *bmhd\nPlanes)
    If *unInterleaveBits
      ...
      For y = 0 To *bmhd\h - 1
        ...
        For z = 0 To *bmhd\nPlanes - 1
          ...
          For x = 0 To bytesPerRow - 1
            C5 = *interleaveBits\b[ C4 + x ]
            C6 = C2 + x << 3
            For bit = 0 To 7
              If C5 & ( 1 << ( 7 - bit ) )
                *unInterleaveBits\b[ C6 + bit ] | C3
              EndIf
            Next
          Next
        Next
    ...
    ProcedureReturn *unInterleaveBits
        ...
        While *chunk
          *chunk\size = GetUInt32BE(*chunk\size)
          ...
          Select *chunk\code
            ...
            Case #ID_BODY
              ; [PBM ] chunk\bytes => *body
              ; [PBM ] chunk\bytes => *bodyUnpacked      => *body
              ; [ILBM] chunk\bytes => *bodyUninterleaved => *body
              ; [ILBM] chunk\bytes => *bodyUnpacked      => *bodyUninterleaved => *body
              Select *mem\format
                Case #ID_ILBM
                  Select *bmhd\compression
                    Case #cmpNone
                      *bodyUninterleaved = UnInterleaveBits(*bmhd, *chunk\bytes)
                      If *bodyUninterleaved
                        *body = *bodyUninterleaved
                      EndIf
                    Case #cmpByteRun1
                      *bodyUnpacked = UnPackBits(*bmhd, *chunk\bytes, *chunk\size)
                      If *bodyUnpacked
                        *bodyUninterleaved = UnInterleaveBits(*bmhd, *bodyUnpacked)
                        If *bodyUninterleaved
                          *body = *bodyUninterleaved
                        EndIf
                      EndIf
